/*
  ...
  1. Changes
    - Drop all existing policies for promotion_claims table
    - Create new simplified policies with unique names
    - Fix INSERT policy to allow users to claim promotions for themselves
    - Ensure no recursion in policy conditions
  ...
  2. Security
    ...
    - Add policies for:
      ...
      - UPDATE: Allow partners and admins to update claims
*/
...
-- Create new simplified policies with unique names
CREATE POLICY "promotion_claims_select_v3"
  ON promotion_claims
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "promotion_claims_insert_v3"
  ON promotion_claims
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "promotion_claims_update_v3"
  ON promotion_claims
  FOR UPDATE
  TO authenticated
  USING (
