...
-- Create function to store role in users table
CREATE OR REPLACE FUNCTION store_role_in_users()
RETURNS trigger AS $$
BEGIN
  UPDATE auth.users
  SET raw_user_meta_data = raw_user_meta_data || jsonb_build_object('role', NEW.name)
  WHERE id IN (
    SELECT user_id FROM user_roles WHERE role_id = NEW.id
  );
  RETURN NEW;
END;
...
CREATE TRIGGER store_role_trigger
