...
-- Get valid provider IDs that exist in auth.users table
...
-- Ensure specific partner has data
DO $$
DECLARE
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM auth.users WHERE id = partner_id) THEN
    ...
    IF NOT EXISTS (
      ...
      -- Add a provider note only if partner exists in auth.users
      IF submission_id IS NOT NULL AND EXISTS (SELECT 1 FROM auth.users WHERE id = partner_id) THEN
        INSERT INTO provider_notes (
          submission_id,
          provider_id,
          content,
