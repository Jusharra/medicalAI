/*
  # Fix Service Preferences Constraint
  
  1. Purpose
    - Fix unique constraint violation
    - Add proper indexes and constraints
    - Update trigger for handling updates
    
  2. Changes
    - Drop and recreate service_preferences table
...
CREATE TABLE service_preferences (
  ...
  CONSTRAINT service_preferences_profile_id_key UNIQUE (profile_id)
);
...
CREATE OR REPLACE FUNCTION handle_service_preferences_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER update_service_preferences_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION handle_service_preferences_updated_at();