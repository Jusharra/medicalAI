...
BEGIN
    ...
    DROP POLICY IF EXISTS "health_metrics_select_policy" ON health_metrics;
    DROP POLICY IF EXISTS "health_metrics_insert_policy" ON health_metrics;
...
BEGIN
    ...
    DROP INDEX IF EXISTS health_metrics_profile_id_idx;
    DROP INDEX IF EXISTS health_metrics_type_idx;
    DROP INDEX IF EXISTS health_metrics_measured_at_idx;
...
CREATE POLICY "health_metrics_view_own"
  ON health_metrics
  FOR SELECT
  TO authenticated
  USING (profile_id = auth.uid());

CREATE POLICY "health_metrics_create_own"
  ON health_metrics
  FOR INSERT
  TO authenticated
  WITH CHECK (profile_id = auth.uid());

CREATE POLICY "chat_messages_view_own"
  ON chat_messages
  FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "chat_messages_create_own"
  ON chat_messages
  FOR INSERT
  TO authenticated
...
CREATE INDEX health_metrics_profile_idx ON health_metrics(profile_id);
CREATE INDEX health_metrics_type_idx ON health_metrics(metric_type);
CREATE INDEX health_metrics_measured_idx ON health_metrics(measured_at);
