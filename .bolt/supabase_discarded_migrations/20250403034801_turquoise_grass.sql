/*
  ...
  1. New Tables
    - `reward_points` - Tracks member point balances
    - `reward_transactions` - Records point earning/redemption history
    - `reward_tiers` - Defines membership tiers and benefits
    - `reward_redemptions` - Tracks reward redemption requests

  2. Security
    - Enable RLS
    - Add policies for members to access their own data
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Members can view their own reward points" ON reward_points;
    DROP POLICY IF EXISTS "Admins have full access to reward points" ON reward_points;
    DROP POLICY IF EXISTS "Members can view their own reward transactions" ON reward_transactions;
    DROP POLICY IF EXISTS "Admins have full access to reward transactions" ON reward_transactions;
    DROP POLICY IF EXISTS "Anyone can view reward tiers" ON reward_tiers;
    DROP POLICY IF EXISTS "Admins have full access to reward tiers" ON reward_tiers;
    DROP POLICY IF EXISTS "Members can view their own reward redemptions" ON reward_redemptions;
    DROP POLICY IF EXISTS "Members can create their own reward redemptions" ON reward_redemptions;
    DROP POLICY IF EXISTS "Admins have full access to reward redemptions" ON reward_redemptions;
EXCEPTION
...
DROP TRIGGER IF EXISTS update_reward_points_updated_at ON reward_points;
...
DROP TRIGGER IF EXISTS update_reward_tiers_updated_at ON reward_tiers;
...
BEGIN
  ...
  IF NEW.transaction_type = 'earn' OR NEW.transaction_type = 'bonus' OR NEW.transaction_type = 'adjustment' THEN
    ...
    IF NEW.points > 0 THEN
      SELECT 
        COALESCE(current_balance, 0) + NEW.points,
        COALESCE(lifetime_earned, 0) + NEW.points,
        COALESCE(lifetime_redeemed, 0)
  ...
  ELSIF NEW.transaction_type = 'redeem' OR NEW.transaction_type = 'expire' THEN
    SELECT 
      COALESCE(current_balance, 0) - ABS(NEW.points),
      COALESCE(lifetime_earned, 0),
      COALESCE(lifetime_redeemed, 0) + ABS(NEW.points)
    INTO point_balance, earned, redeemed
    FROM reward_points
    WHERE profile_id = NEW.profile_id;
  END IF;
  ...
  IF NOT FOUND THEN
    IF NEW.transaction_type = 'earn' OR NEW.transaction_type = 'bonus' OR 
       (NEW.transaction_type = 'adjustment' AND NEW.points > 0) THEN
      point_balance := NEW.points;
      earned := NEW.points;
      redeemed := 0;
    ELSE
      point_balance := 0;
      earned := 0;
      redeemed := ABS(NEW.points);
    END IF;
  END IF;
...
DROP TRIGGER IF EXISTS update_reward_points_balance_trigger ON reward_transactions;
...
-- Insert sample reward tiers if they don't exist
...
VALUES
  (
    ...
    0,
    999,
    '{"perks": ["Basic rewards access", "Standard earning rate", "Monthly newsletter"], "bonus_categories": ["wellness"]}'::jsonb,
    1.0
  ),
  (
    'Silver',
    1000,
    4999,
    '{"perks": ["Priority booking", "10% bonus on earned points", "Quarterly health assessment"], "bonus_categories": ["wellness", "medical"]}'::jsonb,
    1.1
  ),
  (
    'Gold',
    5000,
    9999,
    '{"perks": ["VIP booking", "25% bonus on earned points", "Dedicated health concierge", "Quarterly wellness credit"], "bonus_categories": ["wellness", "medical", "travel"]}'::jsonb,
    1.25
  ),
  (
    'Platinum',
    10000,
    NULL,
    '{"perks": ["Elite booking", "50% bonus on earned points", "Personal health advisor", "Priority access to specialists", "Annual wellness retreat"], "bonus_categories": ["wellness", "medical", "travel", "luxury"]}'::jsonb,
    1.5
  );

-- Insert sample reward points for existing users if they don't have any
INSERT INTO reward_points (
  profile_id,
  current_balance,
  lifetime_earned,
...
-- Insert sample reward transactions if none exist
...
AND NOT EXISTS (
  SELECT 1 FROM reward_transactions LIMIT 1
)
ORDER BY random()
LIMIT 200;

-- Insert sample reward redemptions if none exist
...
AND NOT EXISTS (
  SELECT 1 FROM reward_redemptions LIMIT 1
)
LIMIT 50;